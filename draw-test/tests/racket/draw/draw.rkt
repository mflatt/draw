			...
			(lambda (dc pens pent penx size x y flevel last?)
			  (let* ([ofont (send dc get-font)]
				 ...
				 [obm (send dc get-text-mode)]
                                 [orig-dc dc]
                                 [dc (if current-in-layer?
                                         (cond
                                           [current-squash-layer?
                                            ;; draw into layer with same scale as container,
                                            ;; but create the layer with a stretched scale,
                                            ;; so it will be squashed when drawing back
                                            (send dc scale 2 1)
                                            (define layer-dc (send dc make-layer))
                                            (send dc scale 0.5 1)
                                            (send layer-dc scale 0.5 1)
                                            layer-dc]
                                           [else
                                            (send dc make-layer)])
                                         dc)])
				...
				(send dc set-pen p)))
                            ...
                            (let ([dc (if current-in-layer?
                                          (begin
                                            (send orig-dc draw-layer dc 0 0)
                                            orig-dc)
                                          dc)])
                              (when (and last? 
    ...
    (let ([clock (lambda (clip?)
      ...
      (make-object check-box% "In Layer" hp4
                   (lambda (c e)
                     ...
                     (send canvas refresh)))
      (make-object check-box% "Layer Squash" hp4
                   (lambda (c e)
                     (set! current-squash-layer? (send c get-value))
                     (send canvas refresh)))
